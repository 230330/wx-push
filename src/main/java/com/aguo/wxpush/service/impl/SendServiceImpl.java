 ...
 * @title: SendServiceImpl.java
 * @description: 公众号消息推送实现类
 * @author: hezf
 * @created 2023/3/27 17:01
 */
@Service
public class SendServiceImpl implements SendService {
    ...
    public String sendWeChatMsg() {
        ...
        for (String opedId : configConstant.getOpenidList()) {
            ...
            try {
                ...
                logger.info("wet:{}", wet);
                //获取未来三天天气
                Map<String, String> map = weatherService.getTheNextThreeDaysWeather();
                if (map.isEmpty()) {
            ...
            try {
                ...
                String zhufuyu = "生日快乐，宝贝！";
                if (birthDate1.equals("0")){
                    resultMap.put("zhufuyu",zhufuyu);
                }
            ...
            try {
                ...
                JSONObject togetherMonth = getMonths(date);
                JSONObject togetherYear = getYears(date);
                resultMap.put("togetherYear", togetherYear);
                resultMap.put("togetherMonth", togetherMonth);
                resultMap.put("togetherDate", togetherDate);
                logger.info("togetherYear:{}", togetherYear);
                logger.info("togetherMonth:{}", togetherMonth);
     ...
     * 获取年数
     * */
    private JSONObject getYears(String date) {
        //在一起时间--年
        String togetherYear = "";
        try {
            togetherYear = "第" + DateUtil.yearsBetween(configConstant.getTogetherDate(), date) + "年";
        } catch (ParseException e) {
            logger.error("togetherYear获取失败" + e.getMessage());
        }
        JSONObject togetherDateObj =JsonObjectUtil.packJsonObject(togetherYear,"#FEABB5");
     ...
     * 获取月份数
     * */
    private JSONObject getMonths(String date) {
        //在一起时间--月
        String togetherMonth = "";
        try {
            togetherMonth = "第" + DateUtil.monthsBetween(configConstant.getTogetherDate(), date)+ "月";
        } catch (ParseException e) {
            logger.error("togetherMonth获取失败" + e.getMessage());
        }
        JSONObject togetherDateObj =JsonObjectUtil.packJsonObject(togetherMonth,"#FEABB5");
        return togetherDateObj;
    }
    ...
    private JSONObject togetherDay(String date) {
        ...
        try {
            ...
            logger.error("togetherDay获取失败" + e.getMessage());
        }
    ...
    private JSONObject getBirthday(String birthday, String date) {
        ...
        try {
            ...
            birthDay = DateUtil.birthDayBetween(date, newD);
